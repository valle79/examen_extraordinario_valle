...
| `init/init.sql` | Script maestro: ejecuta los 35 pasos de inicialización |
...
# Verificar que quedó healthy y ver los 35 pasos del init
docker compose ps
...
## 📋 Los 35 pasos de `init.sql`

| Paso | Script | Contenido |
|---|---|---|
| 1/35 | `ddl/01_database.sql` | Creación de la base de datos (FULL, CHECKSUM) |
| 2/35 | `ddl/02_schemas.sql` | Esquemas core, academic, sales, security, audit, utils |
| 3/35 | `ddl/03_tables.sql` | 17 tablas e índices |
| 4/35 | `ddl/04_constraints.sql` | Restricciones 17 PK, 21 FK, 22 UK, 30 CK |
| 5-10/35 | `programmability/functions/*` | 6 funciones de negocio (fn_*.sql) |
| 11-15/35 | `programmability/triggers/*` | 5 triggers: auditoría + comisión automática (trg_*.sql) |
| 16-22/35 | `programmability/views/*` | 7 vistas de reporte (vw_*.sql) |
| 23-31/35 | `programmability/procedures/*` | 9 procedimientos almacenados (usp_*.sql) |
| 32/35 | `security/01_usuarios_permisos.sql` | Perfiles RN-06: logins, usuarios y permisos GRANT/DENY |
| 33/35 | `dml/01_seed_data.sql` | Datos iniciales (seed) |
| 34/35 | `dml/02_test_data.sql` | Datos de prueba vía los procedimientos |
| 35/35 | `dml/03_test_cases.sql` | 18 casos de prueba de reglas de negocio |
